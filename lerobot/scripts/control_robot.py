...
def record(
    ...
    if getattr(cfg.policy, "name", "") == "iql_act":
        print("Using IQL act policy after robotdataset create")
        dataset.meta.info["goal.pick_label"] = cfg.goal_pick_label
        dataset.meta.info["goal.place_label"] = cfg.goal_place_label
        dataset.meta.info["policy"] = "iql_act"
    elif getattr(cfg.policy, "name", "") == "iql_pi0":
        print("Using IQL pi0 policy after robotdataset create")
        dataset.meta.info["goal.pick_label"] = cfg.goal_pick_label
        dataset.meta.info["goal.place_label"] = cfg.goal_place_label
        dataset.meta.info["policy"] = "iql_pi0"
    ...
    warmup_record(robot, events, enable_teleoperation, cfg.warmup_time_s, False, cfg.fps)
    ...
    while True:
        ...
        # sleep for cfg.warmup_time_s seconds to give time to the robot devices to reset
        log_say("Waiting for the next episode", cfg.play_sounds)
        warmup_record(robot, events, enable_teleoperation, cfg.warmup_time_s, False, cfg.fps)
